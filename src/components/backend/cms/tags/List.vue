        ...
        <v-card>
          <v-toolbar color="white" flat>
            ...
            <template v-if="selected.length>0 && canRemove()">
              <v-btn icon @click="onDelete('tags','','APP_UPDATE_ALL_TAGS_DATA', true)">
            ...
            <v-btn v-if="canAdd()" color="primary" fab small @click="addTagModal()">
            ...
            >
              <template v-slot:item.action="{ item }">
                ...
                <v-btn
                  ...
                  rounded
                  class="text-none"
                  @click="onDelete('tags',item.id,'APP_UPDATE_ALL_TAGS_DATA')"
                >eliminar</v-btn>
                <!-- <v-icon small class="mr-2" @click="updateTagModal(item.id)">mdi-pencil</v-icon> -->
...
export default {
  ...
  methods: {
    addTagModal() {
      window.getApp.$emit("APP_ADD_TAG_MODAL");
    },

    updateTagModal(id) {
      this.tag = this.getSingle("tags", id, this.tags);
      window.getApp.$emit("APP_UPDATE_TAG_MODAL", this.tag);
    }
  }
};
</script>
...
#tags-action .v-speed-dial {
  position: fixed;
  z-index: 100;
  bottom: 35px;
}

#tags-action .v-btn--floating {
  position: relative;
}
</style>