      ...
      <v-col cols="12">
        <v-card>
          <v-toolbar flat color="primary" dark>
            <v-toolbar-title>Perfil da conta</v-toolbar-title>
          </v-toolbar>
          <v-tabs vertical>
            <v-tab>
              <v-icon left>mdi-account</v-icon>Informações Pessoais
            </v-tab>
            <v-tab>
              <v-icon left>mdi-lock</v-icon>Segurança e acessos
            </v-tab>
            <v-tab>
            ...
            <v-tab-item>
              <v-card flat>
                <v-card-text>
                  <p>{{authUser.username}} Sed aliquam ultrices mauris. Donec posuere vulputate arcu. Morbi ac felis. Etiam feugiat lorem non metus. Sed a libero.</p>
